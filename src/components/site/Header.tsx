...
const Header = () => {
  ...
  // Función para manejar el clic en el logo (subir al inicio)
  ...
  // FUNCIÓN CORREGIDA: Maneja el scroll al catálogo
        ...
        <div className="lg:hidden bg-background/95 backdrop-blur-xl border-t border-border animate-in fade-in slide-in-from-top-4 duration-300">
          <nav className="container-luxe flex flex-col py-6 gap-5 text-sm uppercase tracking-[0.2em]">
            <Link 
              ...
              className={location.pathname === "/" ? "text-primary font-bold" : "text-foreground/80"}
            >
              Inicio
            </Link>
            ...
            {/* Listado de sub-categorías rápido */}
            <div className="grid grid-cols-2 gap-2 pl-4 border-l border-primary/20">
              {categories.slice(0, 6).map((c) => (
                <Link 
                  ...
                  to={`/categoria/${c.id}`} 
                  onClick={() => setOpen(false)} 
                  className="text-foreground/60 text-[10px] hover:text-primary truncate"
                >
                  · {c.name}
                </Link>
              ))}
            ...
            <a href="#contact" onClick={() => setOpen(false)} className="text-foreground/80 hover:text-primary">
              Contacto
            </a>
