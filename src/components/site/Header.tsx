...
const Header = () => {
  ...
  // Función para hacer scroll suave si estamos en Inicio, o redirigir si no
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 bg-background/80 backdrop-blur-md border-b border-border/60">
      <div className="container-luxe flex items-center justify-between py-4">
        ...
        {/* TÍTULO CLICKABLE (Ya está bien, pero mejoramos el hover) */}
        ...
        <nav className="hidden lg:flex items-center gap-8 text-xs uppercase tracking-[0.2em] font-medium">
          ...
          {/* Usamos un Link en lugar de <a> para que React no recargue toda la página */}
        ...
        {/* INDICADOR BOLIVIA (Responsivo: Oculto en pantallas muy pequeñas) */}
      ...
      {/* MENÚ MÓVIL (Drawer) - Arreglado para que no desaparezca nada */}
      {open && (
        <div className="lg:hidden bg-background/95 backdrop-blur-xl border-t border-border animate-in fade-in slide-in-from-top-4 duration-300">
          <nav className="container-luxe flex flex-col py-6 gap-5 text-sm uppercase tracking-[0.2em]">
            <Link to="/" onClick={() => setOpen(false)} className={location.pathname === "/" ? "text-primary font-bold" : "text-foreground/80"}>
              Inicio
            </Link>
            
            <Link to="/#categorias" onClick={handleCatalogClick} className="text-foreground/80 hover:text-primary">
              Ver Catálogo
            </Link>

            {/* Sub-categorías más compactas para que quepan en pantallas chicas */}
            ...
            <div className="flex justify-between items-center pt-4 border-t border-border/50">
               <a href="tel:+591XXXXXXXX" className="flex items-center gap-2 text-primary font-bold">
